...
class Envelopes(TechnicalIndicator):
    ...
    Attributes:
        ...
        _ti_data (pandas.DataFrame): The calculated indicator. Index is of type
            ...
            ``upper_band_env`` and the ``lower_band_env``.
    ...
    def _calculateTi(self):
        ...
        env = pd.DataFrame(index=self._input_data.index,
                           columns=['upper_band_env', 'lower_band_env'], data=0,
                           dtype='float64')
        ...
        env['upper_band_env'] = (1 + self._shift) * ma
        env['lower_band_env'] = (1 - self._shift) * ma
    ...
    def getTiSignal(self):
        ...
        if self._input_data['close'].iat[-1] > \
                self._ti_data['upper_band_env'].iat[-1]:
            return TRADE_SIGNALS['sell']

        # Price goes below lower band
        elif self._input_data['close'].iat[-1] < \
                self._ti_data['lower_band_env'].iat[-1]:
